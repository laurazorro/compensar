...
vu.sop.load = async function(basePath) {
    try {
        ...
        let tfJsLoad =  vu.sop.loadJs(basePath + '/js/libs/tensorflowjs/3.5.0/tf.min.js');
        await tfJsLoad;

        let tfJsWasmLoad =  vu.sop.loadJs(basePath + '/js/libs/tensorflowjs/3.5.0/tf-backend-wasm.min.js');
        await tfJsWasmLoad;
        ...
        vu.sop.ui.bottomTextBackGroundColor("rgba(255, 102, 0, 0.5)");
...
vu.sop.steps.authFace = async function() {
    ...
    while (true) {
        ...
        try {
            ...
            console.log('endOperation: ',response)
            var endop = response;
            var end = endop.ocr;
            ocrdoc =  parseInt((JSON.stringify(end.number).replace(/\./g, '')).replace(/\"/g, ''));
            console.log(ocrdoc);
            localStorage.setItem('ocrdoc',ocrdoc);
